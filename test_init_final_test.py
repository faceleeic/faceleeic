		...
		if channel != '':			
			...
			for i in range(fixed_bossNum):
				...
				if fixed_bossTime[i] <= now :
					...
					await client.get_channel(channel).send("```" + fixed_bossData[i][0] + fixed_bossData[i][4] + "```" , tts=False)
			...
			for i in range(bossNum):
				...
				if bossTime[i] <= now :
					...
					if bossData[i][6] != '' :
						await client.get_channel(channel).send("```" + bossData[i][0] + bossData[i][4] + '\n<' + bossData[i][6] + '>```' , tts=False)
					else :
						await client.get_channel(channel).send("```" + bossData[i][0] + bossData[i][4] + "```" , tts=False)
						...
						if basicSetting[2] != '0':
							if int(basicSetting[17]) <= bossMungCnt[i] and int(basicSetting[17]) != 0:
								...
								if bossData[i][2] == '0':
									...
									await client.get_channel(channel).send('```다음 ' + bossData[i][0] + ' ' + bossTimeString[i] + '입니다.```', tts=False)
